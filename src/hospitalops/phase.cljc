(ns hospitalops.phase
  "Phase 0->3 staged rollout for the ISIC-861 hospital activities
  operations-coordination actor.

    Phase 0  read-only            -- no writes, still governor-gated.
    Phase 1  assisted-logistics   -- bed assignment allowed,
                                     every write needs human approval.
    Phase 2  assisted-coordination-- adds visitor access, supply request,
                                     staff shift proposals, still approval.
    Phase 3  supervised auto      -- governor-clean, high-confidence
                                     `:coordinate-bed-assignment`/
                                     `:schedule-visitor-access`/
  ...
  needs a human to actually look at it. `hospitalops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently --
  two layers, not one, agree on this."
  (:require [hospitalops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                                           :auto #{}}
   1 {:label "assisted-logistics"     :writes #{:coordinate-bed-assignment}                                                             :auto #{}}
   2 {:label "assisted-coordination"  :writes #{:coordinate-bed-assignment :schedule-visitor-access :coordinate-supply-request
                                               :schedule-staff-shift-proposal}                                             :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:coordinate-bed-assignment :schedule-visitor-access :coordinate-supply-request :schedule-staff-shift-proposal}}})
...
(defn verdict->disposition
  "Map a HospitalGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
