(ns hospitalops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (HospitalAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the HospitalGovernor (:govern)
  ...
  human operator/facilities coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [hospitalops.advisor :as advisor]
            [hospitalops.governor :as governor]
...
(defn build
  ...
  `hospitalops.store/Store`).
  opts:
    :advisor      -- an `hospitalops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HospitalAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; HospitalGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :bed-id (:bed-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :bed-id (:bed-id request) :by (:by approval)}]}
            {:disposition :hold
