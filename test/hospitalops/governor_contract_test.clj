(ns hospitalops.governor-contract-test
...
(deftest bed-assignment-full-flow
  (testing "clean bed-assignment proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-1" :phase 3}
          result (exec-request actor "t1"
                               {:op :coordinate-bed-assignment :bed-id "bed-101" :patch {:status "available" :room "cleaned"}}
                               ctx)]
      (is (some? result))
  ...
  (testing ":flag-safety-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-safety-concern :bed-id "bed-101"
                                :patch {:concern "elevator maintenance required" :confidence 0.99}}
                               ctx)]
      (is (some? result))
...
(deftest unregistered-bed-hard-hold
  (testing "unregistered bed -> permanent HARD hold, never escalates"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-3" :phase 3}]
      (exec-request actor "t3"
                     {:op :coordinate-bed-assignment :bed-id "unknown-bed"
                      :patch {:status "available"}}
                     ctx)
      (is (= 0 (count (store/coordination-log db)))
          "HARD hold must never commit"))))

(deftest unverified-bed-hard-hold
  (testing "registered but unverified bed -> permanent HARD hold"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-4" :phase 3}
          result (exec-request actor "t4"
                               {:op :coordinate-bed-assignment :bed-id "bed-201"
                                :patch {:status "available"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
          "unverified bed must HARD hold"))))
  ...
  (testing "proposal with :effect :commit (not :propose) -> hard hold"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t5"
                               {:op :coordinate-bed-assignment :bed-id "bed-101"
                                :patch {:status "available"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8a"
                     {:op :coordinate-bed-assignment :bed-id "bed-101" :patch {:status "available"}}
                     ctx)
      (exec-request actor "t8b"
                     {:op :coordinate-bed-assignment :bed-id "unknown" :patch {:status "available"}}
                     ctx)
